 ...
 *
 * The bootstrap zip is stored in app/src/main/assets/bootstrap-aarch64.zip.
 * It contains a fully extracted filesystem under the prefix:
 *   data/data/com.iris.assistant/files/
 * which is stripped during extraction, landing everything under [filesDir].
 *
 * Layout after installation:
 *   filesDir/
 *     usr/     ← PREFIX  (bins, libs, etc.)
 *     home/    ← HOME
 *
            ...
            context.assets.open(ASSET_NAME).use { assetStream ->
                ZipInputStream(assetStream.buffered()).use { zis ->
                    while (true) {
                        ...
                        // Strip the full-path prefix to get relative path under filesDir
                        val name = entry.name
                        ...
                        // Remap usr/ → stagingDir so we can atomic-rename at the end
                        val destPath = if (relative.startsWith("usr/")) {
                        ...
                        if (entry.isDirectory) {
                            ...
                            // Mark executables — mirrors Termux installer logic
